<?php 
	...
	class CREATEODREPORT extends FPDF
	{
		...
		function transactions($grossSales,$docdisc,$linedisc,$ardisc,$gcrefund,$payingcustomers,$numTransactions,$itemsold,$voidItems,$first,$last,$totreval,$scharge,$bng)
		{
			$this->Ln(5);
			$this->SetFont("Arial", "B", 6);
			...
			$this->Ln(3);
			$this->SetFont("Arial", "B", 6);
			$this->Cell(42,4,'Beam And Go',0,0,'L');
			$this->Cell(40,4,number_format($bng,2),0,0,'R');
			$this->Ln(3);
			$this->Cell(42,4,'Revalidation Charge',0,0,'L');
			$this->Cell(40,4,number_format($totreval,2),0,0,'R');
			$this->Ln(3);
			$this->Cell(42,4,'Service Charge',0,0,'L');
			$this->Cell(40,4,number_format($scharge->scharge,2),0,0,'R');
			$this->Ln(3);
			...
			$this->Cell(36,4,number_format($ardisc->totalardisc,2),0,0,'R');
			$this->Ln(3);
			$this->Cell(42,4,'Total Discount',0,0,'L');
			$this->Cell(40,4,number_format($linedisc->totallinedisc + $docdisc->totaldocdisc + $ardisc->totalardisc + $totreval,2),0,0,'R');
			$this->Ln(3);
			...
			$netsales = 0;
			$netsales = $grossSales + $scharge->scharge - ($reftotal+$linedisc->totallinedisc + $docdisc->totaldocdisc + $ardisc->totalardisc) + $totreval;
			$this->SetFont("Arial", "", 5);
			$this->Cell(0,6,'----------------------------------------------------------------------------------------------------------------------------------------',0,0,'C');
			$this->Ln(3);
			$this->SetFont("Arial", "B", 6);
	...
	$bng = getBNGSalesEOD($link,$_SESSION['gccashier_store']);

	$totreval = getTotalRevalPaymentEOD($link,$_SESSION['gccashier_store']);
	...
	$scharge = getServiceChargesEOD($link,$_SESSION['gccashier_store']);
	...
	$pdf->transactions($grossSales,$docdisc,$linedisc,$ardisc,$gcrefund,$payingcustomers,$numTransactions,$itemsold,$voidItems,$first,$last,$totreval,$scharge,$bng);

	$link->autocommit(FALSE);


	if(count(getTransactionsEOD($link,$_SESSION['gccashier_store']))> 0 || count(getTransactionBNG($link,$_SESSION['gccashier_store']))>0)
	{
