<?php
	...
	if(isset($_GET['type']))
	{
		...
		$spacebot = 60;
		if(isset($_GET['spacebot']))
		{
			$spacebot = $_GET['spacebot'];
			$spacebot = intval($spacebot);
		}
		...
		switch ($type) {
			...
			case 'bypage':
				...
				{
					echo 'Please input request id.<a href="index.php">Back</a>';
					exit();
				}
				...
				if(isset($_GET['offset']) && trim($_GET['offset'])!='')
				{
					...
					$gcs = getSpecialExteranalByOffset($link,$offset,$id);
				}
				else 
				{
					echo 'Please input offset.<a href="index.php">Back</a>';
					exit();
				}
				break;
			case 'blank':
				...
				<?php
					...
					foreach ($gcs as $gc): 
						...
						else 
						{
							if($start!=0)
							{
								echo '<div style="margin-bottom:'.$spacebot.'px;"></div>';
							}
						}
					?>
					<div class="gcbg" style="marg">		
