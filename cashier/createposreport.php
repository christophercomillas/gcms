<?php 
	...
	if(isset($_GET['d1']) && isset($_GET['d2']))
	{
		...
		if(!isset($_SESSION['gccashier_store']) || !isset($_SESSION['gccashier_id']))
		{
			exit();
		}
	}
	...
	{
		function report_header($td,$tt,$storename,$cashier,$d1,$d2,$trans)
		{
		...
		function transactions($cash,$reval,$ccard,$ar,$docdisc,$linedisc,$ardisc,$gcrefund,$itemsold,$start,$end,$payingcustomers,$numTransactions,$voidItems,$scharge,$linesubref)
		{
			...
			$this->Cell(30,4,'Total Revalidation Charge',0,0,'L');
			$this->Cell(16,4,$scharge->scount,0,0,'R');
			$this->Cell(36,4,number_format($scharge->scharge,2),0,0,'R');	
			$this->Ln(4);

			$gross = ($cash->cash + $reval->cash + $totalcard + $artotal + $scharge->scharge) - $reftotal;
			...
			if(count($docdisc)>0)
			{
				$hasdiscount =1;
				$this->Cell(4,4,'',0,0,'L');
				$this->Cell(30,4,'Subtotal Discount',0,0,'L');
			...
			$this->Cell(36,4,$start,0,0,'R');
			$this->Ln(3);
			$this->Cell(46,4,'Ending Txnno',0,0,'L');
			$this->Cell(36,4,$end,0,0,'R');
			$this->Ln(3);
		}
	}
	...
	$linesubref = getGCRefundLineAndSubByDateAndCashier($link,$_SESSION['gccashier_store'],$_SESSION['gccashier_id'],$d1t,$d2t,$trans);

	$itemsold = itemsSoldPOS($link,$_SESSION['gccashier_store'],$_SESSION['gccashier_id'],$d1t,$d2t,$trans);

	$scharge = getServiceCharges($link,$_SESSION['gccashier_store'],$_SESSION['gccashier_id'],$d1t,$d2t,$trans);
	...
	$start = empty($start->trans_number) ? '00000000' : $start->trans_number;

	$end = transactionStartEndPOS($link,$_SESSION['gccashier_store'],$_SESSION['gccashier_id'],$d1t,$d2t,$trans,'DESC');
	$end = empty($end->trans_number) ? '00000000' : $end->trans_number;
	$pdf->report_header(_dateFromSql($todays_date),_timeFormat($todays_time),$storename,$cashier,$d1,$d2,$trans);
	...
	//$pdf->Output();
	$pdf->Output('../reports/pos/pos_report'.$_SESSION['gccashier_store'].'.pdf','F');
