...
class Zreport extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetTextColor(28, 28, 28);
		$this->Cell(0, 4,'End of Day Sales', 0, 0, "R");
		...
		");
	   	...
	   	if($n>0){
	   		while($row = $query->fetch_object()){
				...
				$this->Ln(3);
				$this->Cell(102);
				$this->Cell(5, 2, 'Cashier: ', 0, 0, "R");		
				$this->Cell(5, 2, ucwords($row->ss_firstname.' '.$row->ss_lastname), 0, 0, "L");
				$this->Ln(3);
				$this->Cell(102);
				$this->Cell(5, 2, 'Time: ', 0, 0, "R");		
				$this->Cell(5, 2,_timeFormat($row->trans_datetime), 0, 0, "L");
				$this->ln(5);				
			}
	...
	function TransactionDetails($link,$todays_date,$salescredit,$salescash,$returnedTotal){
		...
		$this->Cell(10, 4,number_format($salescredit,2), 0, 0, "L");	
		$this->Ln(3);
		$this->Cell(100);
...
$getGCReturned = getGCReturnedEOD($link,$todays_date,$_SESSION['gc_store']);
$returnedTotal = 0;
foreach ($getGCReturned as $denom) {
	$returnedTotal += $denom->denomination;
}
// $gcRefund = refundGC($link,$todays_date);
$salescredit = getonSalesEOD($link,$todays_date,'2');
$salescash = getonSalesEOD($link,$todays_date,'1');
