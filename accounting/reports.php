<?php
	...
	class REPORTS extends FPDF
	{
		...
		function subheaderpromo($id,$data)
		{
			...
			$this->Cell(28,5,'Release #.: ',0,0,'R');
			$this->SetFont("Arial", "", 10);
			$this->Cell(86,5,sprintf("%03d",$data->reqap_trnum),0,0,'L');
			$this->SetFont("Arial", "B", 10);
		...
		function subfooterpromo($data,$totcount)
		{
			...
			$this->Ln(8);
			$this->SetFont("Arial", "B", 10);
			$this->Cell(80,	8,ucwords($data->spexgc_receviedby),0,0,'C');
			$this->Cell(34,8,'',0,0,'C');
			$this->Cell(60,8,ucwords($data->rby),0,0,'C');
			$this->Ln(4);
			$this->SetFont("Arial", "", 10);
